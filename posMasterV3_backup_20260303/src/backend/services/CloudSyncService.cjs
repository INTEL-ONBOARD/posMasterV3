...
class CloudSyncService {
    constructor() {
        ...
        // Startup buffer: absorbs notifyDataChange() calls that arrive before MySQL
        // is initialized. Drained once ensureMySQLSchema() completes (Fix 4).
        this._startupBuffer = [];
        ...
        // Sync lock — Promise-chain mutex (Fix 11: replaces the racy boolean lock)
    ...
    /**
     * Acquire sync lock using a Promise-chain mutex (Fix 11).
     * Each caller appends to _lockChain and receives a release function.
     * @param {number} timeout - Max time to wait in ms (default: 30000)
     * @returns {Promise<Function|false>} release function, or false on timeout
     */
    _acquireSyncLock(timeout = 30000) {
        let release;
        const acquired = new Promise(resolve => { release = resolve; });

        // Chain: wait for current tail, then hand lock to this caller
        const previous = this._lockChain;
        this._lockChain = previous.then(() => acquired);
    ...
    /**
     * Release sync lock — call the function returned by _acquireSyncLock
     * Kept as a no-op wrapper so existing call sites don't break.
     */
    _releaseSyncLock(release) {
    ...
    async initializeMySQL() {
        ...
        try {
            ...
            if (connected) {
                ...
                // Fix 4: drain the startup buffer — queue any changes that arrived before
                // MySQL was ready so they are not silently lost
                if (this._startupBuffer.length > 0) {
                    console.log(`[CloudSync] Draining ${this._startupBuffer.length} startup-buffered changes...`);
                    const buffered = this._startupBuffer.splice(0, this._startupBuffer.length);
                    for (const { tableName, operation, record, recordId } of buffered) {
                        this.queueChange(tableName, operation, record, recordId).catch(err =>
                            console.warn('[CloudSync] Startup buffer drain error:', err.message)
                        );
                    }
                }
    ...
    async syncSingleChange(change) {
        ...
        if (operation === 'DELETE') {
            ...
            // AND table-specific exclusions (e.g. sku/item_name are virtual JOINs in
            // sales_items/restock_items but real columns in items) — Fix 8
    ...
    async syncPendingChanges() {
        ...
        // Use Promise-chain mutex to prevent concurrent syncs (Fix 11)
        const release = await this._acquireSyncLock(5000);
        if (!release) {
            return { status: 'skipped', reason: 'already_syncing' };
        }

        if (this.isSyncing) {
            this._releaseSyncLock(release);
            return { status: 'skipped', reason: 'already_syncing' };
        }
    ...
    async performFullSync(options = {}) {
        // Use Promise-chain mutex to prevent concurrent syncs (Fix 11)
        const release = await this._acquireSyncLock(10000);
        if (!release) {
        ...
        // Fix 15: check schema compatibility before syncing (blocks mismatched devices)
        const schemaCheck = await this.checkSchemaCompatibility();
        if (!schemaCheck.compatible) {
            this._releaseSyncLock(release);
            return {
                status: 'schema_mismatch',
                localVersion: schemaCheck.localVersion,
                cloudVersion: schemaCheck.cloudVersion,
        ...
        for (const tableName of TABLES_TO_SYNC) {
            ...
            try {
                ...
                // Fix 7: active_sessions must use pullFromCloud exclusively (keyed by user_id).
                // bidirectionalSyncTable keys by `id` which creates duplicate rows, breaking
                // single-device enforcement. RealTimeSyncService polls every 3 s for pushes.
        ...
        // Fix 12: include failedCount so UI can show a "sync failures" badge
        const totalRecordsUpdated = results.uploaded + results.downloaded;
        const failedCount = this.syncQueueRepo ? this.syncQueueRepo.getFailedCount() : 0;
        broadcastSyncComplete({
    ...
    async pushRecordToCloud(tableName, record, columns) {
        ...
        try {
            ...
            // Fix 5: write cloud_id back to SQLite after a successful push so the record
            // is not treated as new on every subsequent sync cycle (preventing duplicates).
            // cloud_id = the record's id in MySQL (same value since we INSERT with explicit id).
            try {
                const db = getDatabase();
                if (db) {
                    const tableInfo = db.prepare(`PRAGMA table_info(${tableName})`).all();
                    const hasCloudId = tableInfo.some(col => col.name === 'cloud_id');
                    const hasSyncedAt = tableInfo.some(col => col.name === 'synced_at');
                    if (hasCloudId && hasSyncedAt) {
                        db.prepare(`UPDATE ${tableName} SET cloud_id = ?, sync_status = 'synced', synced_at = ? WHERE id = ?`)
                            .run(String(record.id), nowISO(), record.id);
                    } else if (hasCloudId) {
                        db.prepare(`UPDATE ${tableName} SET cloud_id = ?, sync_status = 'synced' WHERE id = ?`)
                            .run(String(record.id), record.id);
                    } else {
                        this.updateLocalSyncStatus(tableName, record.id, 'synced');
                    }
                }
            } catch (writebackErr) {
                // Non-fatal — local writeback is best-effort
                console.warn(`[CloudSync] cloud_id writeback failed for ${tableName} ${record.id}:`, writebackErr.message);
            }
    ...
    async pullRecordToLocal(tableName, cloudRecord, localColumns, primaryKey, skipBroadcast = false) {
        ...
        // Fix 26: for items table, use ON CONFLICT(id) DO UPDATE SET ... instead of
        // INSERT OR REPLACE. INSERT OR REPLACE deletes the old row and reinserts it,
        // destroying the local id and breaking all FK references (sales_items, stock, etc.).
        // The ON CONFLICT-based upsert preserves the existing row's id.
        // For sku conflicts (different id, same sku), fall through to the sku-conflict handler below.
        const query = `
    ...
    async pullFromCloud(tableName) {
        ...
        try {
            ...
            // Fix 21: collect records that failed FK checks for a single retry pass
            const retryAfterParents = [];
                ...
                try {
                    ...
                    if (tableName === 'active_sessions' && record.user_id) {
                        ...
                        if (existingSession && !sessionChanged) {
                            ...
                            // Fix 18: the original condition was inverted — localTime > cloudTime - GRACE
                            // kept the local record even when cloud was genuinely newer. Correct logic:
                            // skip the pull only when local is clearly ahead of cloud (allowing GRACE for
                            // minor clock skew). When cloud is newer, fall through to update the local record.
                            if (existingSession.updated_at && record.updated_at) {
                                ...
                                // Skip pull only if local is strictly newer than cloud (minus grace window)
                    ...
                    // Fix 26: use ON CONFLICT(primaryKey) DO UPDATE SET for ALL tables including items.
                    // INSERT OR REPLACE would delete the local row (destroying its id) which breaks
                    // all FK references in sales_items, restock_items, stock, etc.
                    // sku conflicts are handled separately in the UNIQUE constraint catch block below.
                    const query = `
                        INSERT INTO ${tableName} (${commonColumns.join(', ')})
                        VALUES (${placeholders})
                        ON CONFLICT(${primaryKey}) DO UPDATE SET ${updateSet}
                    ...
                    // Handle FOREIGN KEY constraint failures gracefully (Fix 21)
                    // This happens when syncing user_settings/login_history before their parent user exists
                    if (error.message && error.message.includes('FOREIGN KEY constraint failed')) {
                        // Requeue for one retry pass after the main loop completes.
                        // This handles out-of-order parent/child arrival during initial sync.
                        retryAfterParents.push({ record, commonColumns, primaryKey, placeholders: commonColumns.map(() => '?').join(', '), updateSet: commonColumns.filter(col => col !== primaryKey).map(col => `${col} = excluded.${col}`).join(', ') });
                        console.warn(`[CloudSync] FK constraint for ${tableName} record ${record[primaryKey]} — will retry after parents`);
            ...
            // Fix 21: retry records that failed FK checks in the first pass.
            // By now, parent records should have been inserted, so children can succeed.
            if (retryAfterParents.length > 0) {
                console.log(`[CloudSync] ${tableName}: retrying ${retryAfterParents.length} FK-failed records...`);
                for (const { record: r, commonColumns: cc, primaryKey: pk } of retryAfterParents) {
                    try {
                        const vals = cc.map(col => {
                            const val = r[col];
                            if (val === null || val === undefined) return null;
                            if (typeof val === 'boolean') return val ? 1 : 0;
                            if (val instanceof Date) return val.toISOString();
                        ...
                        const updSet = cc.filter(col => col !== pk).map(col => `${col} = excluded.${col}`).join(', ');
                        db.prepare(`
                            INSERT INTO ${tableName} (${cc.join(', ')})
                            VALUES (${cc.map(() => '?').join(', ')})
                            ON CONFLICT(${pk}) DO UPDATE SET ${updSet}
                        `).run(...vals);
                        result.downloaded++;
                        result.actuallyChanged++;
                    } catch (retryErr) {
                        console.error(`[CloudSync] Retry FK failed for ${tableName} record ${r[primaryKey]}:`, retryErr.message);
                    }
                }
            }
            ...
            for (const record of localRecords) {
                try {
                    const values = columns.map(col => {
                        ...
                        // For users table, ensure roles is always a valid JSON string.
                        // Fix 6: use '["cashier"]' (not '["user"]') to match pushRecordToCloud default.
                        if (tableName === 'users' && col === 'roles') {
                            if (!val || val === '' || val === 'null' || val === '[]') {
                                val = record.username === 'admin' ? '["admin"]' : '["cashier"]';
    ...
    async createMySQLSchema() {
        ...
        // Fix 13: schema_version table — tracks the SQLite migration level that was
        // in effect when MySQL schema was last set up. Used by checkSchemaCompatibility().
        ...
        // Seed schema_version row (INSERT IGNORE so it only runs once)
        try {
            await executeQuery(`INSERT IGNORE INTO schema_version (id, version) VALUES (1, 0)`);
        } catch (err) {
            console.warn('[CloudSync] Could not seed schema_version row:', err.message);
        }
    ...
    /**
     * Check that the local SQLite schema version matches the cloud MySQL schema version.
     * Fix 15: prevents syncing when one device is on a newer schema than another.
     * - If local is ahead of cloud  → upgrade MySQL schema, then proceed
     * - If local is behind cloud    → block sync and emit schema-mismatch event
     * - If equal                    → proceed normally
     * @returns {{ compatible: boolean, localVersion: number, cloudVersion: number, reason?: string }}
     */
    async checkSchemaCompatibility() {
        try {
            const { getSchemaVersion } = require('../database/migrator.cjs');
            const db = getDatabase();
            const localVersion = getSchemaVersion(db);

            const rows = await executeQuery('SELECT version FROM schema_version WHERE id = 1');
            const cloudVersion = (rows && rows.length > 0) ? (rows[0].version || 0) : 0;

            if (localVersion === cloudVersion) {
                return { compatible: true, localVersion, cloudVersion };
            }

            if (localVersion > cloudVersion) {
                // This device is ahead — run ensureSchema to push new tables/columns to MySQL
                console.log(`[CloudSync] Schema upgrade: local=${localVersion} > cloud=${cloudVersion}. Upgrading MySQL...`);
                await this.ensureMySQLSchema();
                await executeQuery(
                    'UPDATE schema_version SET version = ?, applied_at = NOW() WHERE id = 1',
                    [localVersion]
                );
                console.log(`[CloudSync] MySQL schema upgraded to version ${localVersion}`);
                return { compatible: true, localVersion, cloudVersion, upgraded: true };
            }

            // localVersion < cloudVersion — this device is behind
            const reason = `Local schema v${localVersion} is behind cloud v${cloudVersion}. Please update the app on this device.`;
            console.warn(`[CloudSync] Schema mismatch: ${reason}`);

            // Broadcast so UI can show "update required" notice (Fix 16)
            broadcastEvent('sync:schema-mismatch', { localVersion, cloudVersion, reason });

            return { compatible: false, localVersion, cloudVersion, reason };
        } catch (err) {
            // If schema_version table doesn't exist yet (first run), treat as compatible
            console.warn('[CloudSync] checkSchemaCompatibility failed (first run?):', err.message);
            return { compatible: true, localVersion: 0, cloudVersion: 0 };
        }
    }
 ...
 * Fix 4: if MySQL is not yet initialized, buffer the change so it is not lost
 */
function notifyDataChange(tableName, operation, record, recordId) {
    try {
        const service = getCloudSyncService();
        if (!service.mysqlInitialized) {
            // Buffer the change — will be drained once MySQL finishes initializing
            const MAX_BUFFER = 500;
            if (service._startupBuffer.length < MAX_BUFFER) {
                service._startupBuffer.push({ tableName, operation, record, recordId });
            } else {
                console.warn('[CloudSync] Startup buffer full, dropping change:', operation, tableName, recordId);
            }
            return;
        }
