...
class SalesRepository extends BaseRepository {
    ...
    createWithItems(data, items = []) {
        ...
        // Execute the transaction first — only notify/broadcast AFTER it commits (Fix 2)
        const fullSale = transaction();

        // Notify CloudSync of the new sale
        notifyDataChange('sales_transactions', 'INSERT', fullSale, fullSale.id);

        // Broadcast to UI for immediate update
        broadcastDataChange('sales_transactions', 'INSERT', fullSale.id, fullSale);
    ...
    completeHeldOrder(id, updateData = {}) {
        ...
        const transaction = this.db.transaction(() => {
            ...
            // Update sale (BaseRepository.update broadcasts via its own path — but this is inside
            // a transaction so we suppress that by not relying on it for the main notification)
            this.db.prepare(`
                UPDATE sales_transactions SET is_held = 0, status = 'completed', updated_at = ?
                WHERE id = ?
            `).run(nowISO(), id);
            ...
            if (Object.keys(updateData).length > 0) {
                ...
                this.db.prepare(`UPDATE sales_transactions SET ${setClause} WHERE id = ?`).run(...vals, id);
            }
        ...
        // Execute the transaction first — only notify/broadcast AFTER it commits (Fix 2)
        const completedSale = transaction();
        ...
        if (itemCount > 0) {
            broadcastBatchChange('stock', itemCount, 'SALE_COMPLETE');
        }
    ...
    cancelSale(id) {
        ...
        const transaction = this.db.transaction(() => {
            ...
            // Update sale status
            this.db.prepare(`UPDATE sales_transactions SET status = 'cancelled', updated_at = ? WHERE id = ?`).run(nowISO(), id);
        ...
        if (!result || !saleSnapshot) return false;

        const cancelledSale = this.getFullDetails(id);
        notifyDataChange('sales_transactions', 'UPDATE', cancelledSale || { id }, id);
        broadcastDataChange('sales_transactions', 'UPDATE', id, cancelledSale);

        if (saleSnapshot.member_id && !saleSnapshot.is_held) {
            const updatedMember = this.db.prepare('SELECT * FROM members WHERE id = ?').get(saleSnapshot.member_id);
            if (updatedMember) {
                notifyDataChange('members', 'UPDATE', updatedMember, saleSnapshot.member_id);
            }
            broadcastDataChange('members', 'UPDATE', saleSnapshot.member_id, null);
        }

        if (saleSnapshot.items.length > 0) {
            broadcastBatchChange('stock', saleSnapshot.items.length, 'SALE_CANCEL');
        }
    ...
    returnSaleItems(saleId, itemsToReturn, reason, returnedBy) {
        ...
        const transaction = this.db.transaction(() => {
            ...
            for (const item of itemsToReturn) {
                ...
                insertedReturnIds.push(result.lastInsertRowid);
            }
            ...
            if (sale) {
                ...
                const previouslyReturned = existingReturns
                    .filter(r => !insertedReturnIds.includes(r.id))
                    .reduce((sum, r) => sum + r.quantity, 0);
                const allReturned = previouslyReturned + totalReturnedQty >= totalOriginalQty;
                this.db.prepare(`UPDATE sales_transactions SET status = ?, updated_at = ? WHERE id = ?`)
                    .run(allReturned ? 'returned' : 'partial_return', nowISO(), saleId);
            }
        });
        ...
        // Fetch the committed return records and notify CloudSync (Fix 24)
        const insertedReturns = insertedReturnIds.map(rid => returnRepo.findById(rid)).filter(Boolean);
        for (const ret of insertedReturns) {
            notifyDataChange('returned_items', 'INSERT', ret, ret.id);
        }

        // Notify sale status update
        const updatedSale = this.findById(saleId);
        if (updatedSale) {
            notifyDataChange('sales_transactions', 'UPDATE', updatedSale, saleId);
        }
