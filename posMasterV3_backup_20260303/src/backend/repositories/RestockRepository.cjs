...
class RestockRepository extends BaseRepository {
    ...
    createWithItems(data, addedItems = [], returnItems = []) {
        ...
        const fullRestock = transaction();

        // Notify CloudSync of the new restock
        notifyDataChange('restock_transactions', 'INSERT', fullRestock, fullRestock.id);

        // Broadcast to UI for immediate update
        broadcastDataChange('restock_transactions', 'INSERT', fullRestock.id, fullRestock);
