...
class SyncQueueRepository extends BaseRepository {
    ...
    /**
     * Get the count of permanently failed items (Fix 12)
     * Items that have exhausted all retries and are stuck in 'failed' status.
     * Exposed so the UI can show a "failed sync" badge to the user.
     * @returns {number}
     */
    getFailedCount() {
        try {
            const row = this.db.prepare(`SELECT COUNT(*) as count FROM sync_queue WHERE status = 'failed'`).get();
            return row?.count || 0;
        } catch (err) {
            console.warn('[SyncQueueRepository] getFailedCount error:', err.message);
            return 0;
        }
    }
