...
class DataStore {
    constructor() {
        ...
        // Schema mismatch state (Fix 16) — populated when cloud schema is ahead of local
        this.schemaMismatch = { detected: false, localVersion: 0, cloudVersion: 0, reason: null };
    ...
    _setupEventListeners() {
        ...
        // Listen for schema mismatch events (Fix 16)
        // Fired when this device's app is behind the cloud schema — sync is paused
        if (window.electronAPI.onSchemaMismatch) {
            const cleanup = window.electronAPI.onSchemaMismatch((data) => {
                console.warn('[DataStore] Schema mismatch detected:', data);
                this.schemaMismatch = {
                    detected: true,
                ...
                // Notify subscribers registered under 'schema_mismatch' key
                // Components can read dataStore.schemaMismatch for the full payload
                this._notifySubscribers('schema_mismatch');
            });
            if (typeof cleanup === 'function') {
                this._eventCleanupFunctions.push(cleanup);
            }
