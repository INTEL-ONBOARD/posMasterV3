...
contextBridge.exposeInMainWorld("electronAPI", {
    ...
    /**
     * Listen for schema mismatch event (Fix 16)
     * Fired when this device's SQLite schema version is behind the cloud MySQL version.
     * The callback receives: { localVersion, cloudVersion, reason }
     * UI should show an "app update required — sync paused" notice.
     */
    onSchemaMismatch: (callback) => {
        const handler = (event, data) => callback(data);
        ipcRenderer.on("sync:schema-mismatch", handler);
        return () => ipcRenderer.removeListener("sync:schema-mismatch", handler);
    },
