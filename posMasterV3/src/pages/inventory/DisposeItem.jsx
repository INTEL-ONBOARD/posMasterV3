...
function DisposeItem() {
  ...
  const [formDataDisposeItem, setFormDataDisposeItem] =
    useState(INITIAL_DISPOSE_FORM);
  ...
  // Get current date for default dispose date
  ...
  const removeItemFromTable = useCallback(
    (itemId) => {
      ...
      if (selectedItemForDetails?.id === itemId) {
        ...
        setFormDataDisposeItem(INITIAL_DISPOSE_FORM);
        setStockEntries([]);
      }
    },
  ...
  const selectItemForDetails = useCallback(
    (item) => {
      ...
      setFormDataDisposeItem((prev) => ({
        ...prev,
        dispose_date: getCurrentDate(),
      }));
  ...
  const handleDisposeItemInputChange = useCallback(
    (e) => {
      const { name, value } = e.target;
      setFormDataDisposeItem((prev) => ({
        ...prev,
  ...
  const validateDisposeForm = () => {
    const errors = {};

    if (!formDataDisposeItem.quantity.trim()) {
      errors.quantity = "Quantity is required";
    } else if (
      isNaN(formDataDisposeItem.quantity) ||
      parseFloat(formDataDisposeItem.quantity) <= 0
    ) {
      errors.quantity = "Quantity must be a positive number";
    } else if (
      selectedItemForDetails &&
      parseFloat(formDataDisposeItem.quantity) >
        selectedItemForDetails.current_qty
    ) {
      errors.quantity = "Dispose quantity cannot exceed available stock";
    }

    if (!formDataDisposeItem.dispose_description.trim()) {
      errors.dispose_description = "Dispose description is required";
    }

    if (!formDataDisposeItem.dispose_date.trim()) {
      errors.dispose_date = "Dispose date is required";
    }
      ...
      <div className="bg-gray-300 w-[calc(30.5rem)] h-[calc(100vh-2rem)] p-2 z-10">
        <div className="flex flex-col h-[46rem] gap-3">
          ...
          <div className="bg-white">
            <button
              onClick={() =>
                setOpenFormBlock(openFormBlock === "dispose" ? "" : "dispose")
              }
            ...
            >
              ...
              {openFormBlock === "dispose" ? <ChevronUp /> : <ChevronDown />}
            </button>
                ...
                <div className="">
                  ...
                  <div className="mt-4">
                    ...
                    <input
                      type="date"
                      name="dispose_date"
                      value={formDataDisposeItem.dispose_date}
                      onChange={handleDisposeItemInputChange}
                      disabled={!selectedItemForDetails}
                      className={`w-full px-3 py-2 bg-[#F8F8F8] border ${
                        formDisposeErrors.dispose_date
                          ? "border-red-500 focus:ring-red-500"
                          : "border-[#EBEBEB] focus:ring-blue-500"
                      } focus:outline-none focus:ring-2 focus:border-transparent ${
                        !selectedItemForDetails
                  ...
                  <div className="mt-4">
                    ...
                    <textarea
                      ...
                      type="text"
                      value={formDataDisposeItem.dispose_description}
                      onChange={handleDisposeItemInputChange}
                      disabled={!selectedItemForDetails}
                      placeholder="Enter disposal reason and details..."
                      rows={3}
                      className={`w-full mt-2 px-3 py-2 bg-[#F8F8F8] border ${
                        formDisposeErrors.dispose_description
                          ? "border-red-500 focus:ring-red-500"
                          : "border-[#EBEBEB] focus:ring-blue-500"
                      } focus:outline-none focus:ring-2 focus:border-transparent ${
                        !selectedItemForDetails
      ...
      <div className="flex-1 bg-gray-50 p-6 shadow-md rounded-md">
        ...
        {selectedItemsForTable.length === 0 && (
          <div className="flex flex-col items-center justify-center h-64 text-gray-500">
            <p className="text-lg mb-2">No items selected for disposal</p>
            <p className="text-sm">
